...
public class PlayerHeadResolver {
    ...
    public static void resolve(UUID uuid, Texture texture) {
        if (imageCache.containsKey(uuid)) {
            VertexMain.client.execute(() -> VertexMain.client.getTextureManager().registerTexture(texture, imageCache.get(uuid)));
            return;
        }
        imageCache.put(uuid, EMPTY);
        VertexMain.client.execute(() -> VertexMain.client.getTextureManager().registerTexture(texture, EMPTY));
        ...
        VertexMain.log(Level.DEBUG, "Getting player head for", uuid);
        client.sendAsync(hr, HttpResponse.BodyHandlers.ofByteArray()).thenAccept(httpResponse -> {
            try {
                ...
                VertexMain.client.execute(() -> VertexMain.client.getTextureManager().registerTexture(texture, nib));
