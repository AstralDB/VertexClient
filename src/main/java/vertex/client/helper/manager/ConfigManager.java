...
public class ConfigManager {
    ...
    static {
        CONFIG_FILE = new File(VertexMain.BASE, "config.sip");
    }
    ...
    public static void saveState() {
        if (!loaded || !enabled) {
            VertexMain.log(Level.INFO, "Not saving config because we didn't load it yet");
            return;
        }
        VertexMain.log(Level.INFO, "Saving state");
        ...
        try {
            ...
            e.printStackTrace();
            VertexMain.log(Level.ERROR, "Failed to save config!");
        }
