...
public class Rotations {
    ...
    public static void lookAtPositionSmoothServerSide(Vec3d target, double laziness) {
        double delX = target.x - Objects.requireNonNull(VertexMain.client.player).getX();
        double delZ = target.z - VertexMain.client.player.getZ();
        double delY = target.y - (VertexMain.client.player.getY() + VertexMain.client.player.getEyeHeight(VertexMain.client.player.getPose()));
    ...
    public static void lookAtPositionSmooth(Vec3d target, double laziness) {
        ...
        if (fl.isEnabled()) {
            ...
            // setting yaw
            delta = MathHelper.wrapDegrees(required - VertexMain.client.player.getYaw());
            speed = Math.abs(delta / laziness);
            add = speed * (delta >= 0 ? 1 : -1);
            if ((add >= 0 && add > delta) || (add < 0 && add < delta)) {
                add = delta;
            }
            VertexMain.client.player.setYaw(VertexMain.client.player.getYaw() + (float) add);

            // setting pitch
            required = degTan;
            delta = MathHelper.wrapDegrees(required - VertexMain.client.player.getPitch());
            speed = Math.abs(delta / laziness);
            add = speed * (delta >= 0 ? 1 : -1);
            if ((add >= 0 && add > delta) || (add < 0 && add < delta)) {
                add = delta;
            }
            VertexMain.client.player.setPitch(VertexMain.client.player.getPitch() + (float) add);
        }

    }

    public static Vec2f getPitchYaw(Vec3d targetV3) {
        return getPitchYawFromOtherEntity(Objects.requireNonNull(VertexMain.client.player).getEyePos(), targetV3);
    }
