...
public class ItemData extends Command {
    ...
    public void onExecute(String[] args) throws CommandException {
        ...
        if (VertexMain.client.interactionManager.hasCreativeInventory() && !onlyShow) {
            giveItem(item);
            message("Item copied.");
        } else {
            NbtCompound tag = item.getNbt();
            String nbt = tag == null ? "" : tag.asString();
            VertexMain.client.keyboard.setClipboard(nbt);
            message("Nbt copied.");
        }
    ...
    private void giveItem(ItemStack stack) {
        int slot = VertexMain.client.player.getInventory().getEmptySlot();
        if (slot < 0) {
