...
public class AntiAnvil extends Module {
    ...
    @Override
    public void tick() {
        BlockPos currentPos = Objects.requireNonNull(VertexMain.client.player).getBlockPos();
        Vec3d ppos = VertexMain.client.player.getPos();
        List<Entity> anvils = StreamSupport.stream(Objects.requireNonNull(VertexMain.client.world).getEntities().spliterator(), false).filter(entity -> {
