...
public class Backtrack extends Module {
    ...
    boolean shouldBacktrack() {
        return InputUtil.isKeyPressed(VertexMain.client.getWindow().getHandle(), GLFW.GLFW_KEY_LEFT_ALT) && VertexMain.client.currentScreen == null;
    }

    void shouldCommit() {
        boolean a = !committed && InputUtil.isKeyPressed(VertexMain.client.getWindow().getHandle(),
                GLFW.GLFW_KEY_ENTER
        ) && VertexMain.client.currentScreen == null;
        if (a) {
            committed = true;
        }
    }
    ...
    void moveTo(PositionEntry e) {
        VertexMain.client.player.updatePosition(e.pos.x, e.pos.y, e.pos.z);
        VertexMain.client.player.setPitch((float) e.pitch);
        VertexMain.client.player.setYaw((float) e.yaw);
        VertexMain.client.player.setVelocity(e.vel);
    }
    ...
    public void disable() {
        entries.clear();
        committed = false;
        VertexMain.client.player.setNoGravity(false);
    }
    ...
    public void onFastTick() {
        ...
        if (!shouldBacktrack() && !committed) {
            entries.add(new PositionEntry(Utils.getInterpolatedEntityPosition(VertexMain.client.player),
                    VertexMain.client.player.getVelocity(),
                    VertexMain.client.player.getPitch(),
                    VertexMain.client.player.getYaw()
            ));
        } else if (committed) {
            VertexMain.client.player.setNoGravity(true);
