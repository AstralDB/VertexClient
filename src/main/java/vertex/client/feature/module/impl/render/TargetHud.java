...
import vertex.client.helper.manager.AttackManager;
...
public class TargetHud extends Module {
    ...
    boolean isApplicable(Entity check) {
        if (check == VertexMain.client.player) {
            return false;
        }
        if (check.distanceTo(VertexMain.client.player) > 64) {
            return false;
        }
    ...
    public void tick() {
        ...
        List<Entity> entitiesQueue = StreamSupport.stream(Objects.requireNonNull(VertexMain.client.world).getEntities().spliterator(), false)
                .filter(this::isApplicable)
                .sorted(Comparator.comparingDouble(value -> value.getPos().distanceTo(Objects.requireNonNull(VertexMain.client.player).getPos())))
                .toList();
    ...
    public void draw(MatrixStack stack) {
        ...
        if (re != null) {
            ...
            PlayerListEntry ple = Objects.requireNonNull(VertexMain.client.getNetworkHandler()).getPlayerListEntry(entity.getUuid());
