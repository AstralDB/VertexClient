...
import vertex.client.mixin.render.IWorldRendererMixin;
...
public class BlockHighlighting extends Module {
    ...
    public void renderEntry(MatrixStack stack, Long2ObjectMap.Entry<SortedSet<BlockBreakingInfo>> e) {
        ...
        BlockState bs = VertexMain.client.world.getBlockState(kv);
        VoxelShape vs = bs.getOutlineShape(VertexMain.client.world, kv);
        if (vs.isEmpty()) {
            return;
        }
    ...
    public void onWorldRender(MatrixStack matrices) {
        for (Long2ObjectMap.Entry<SortedSet<BlockBreakingInfo>> sortedSetEntry : ((IWorldRendererMixin) VertexMain.client.worldRenderer).getBlockBreakingProgressions()
                .long2ObjectEntrySet()) {
