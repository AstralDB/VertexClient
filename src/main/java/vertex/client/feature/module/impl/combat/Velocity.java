...
import vertex.client.mixin.network.IEntityVelocityUpdateS2CPacketMixin;
...
public class Velocity extends Module {
    ...
    public Velocity() {
        ...
        Events.registerEventHandler(EventType.PACKET_RECEIVE, event -> {
            if (!this.isEnabled() || VertexMain.client.player == null) {
                return;
            }
            PacketEvent pe = (PacketEvent) event;
            if (pe.getPacket() instanceof EntityVelocityUpdateS2CPacket packet && packet.getId() == VertexMain.client.player.getId()) {
