...
public class CaveMapper extends Module {
    ...
    public void onFastTick() {
        for (int i = 0; i < 10; i++) {
            ...
            for (BlockPos pos : new BlockPos[] { right, left, fw, bw, up, down }) {
                ...
                int y = pos.getY();
                while (!Objects.requireNonNull(VertexMain.client.world).isOutOfHeightLimit(y)) {
                ...
                } else if (bs(pos).isFullCube(VertexMain.client.world, pos) && circ.stream()
    ...
    BlockState bs(BlockPos bp) {
        return Objects.requireNonNull(VertexMain.client.world).getBlockState(bp);
    }
    ...
    public void enable() {
        ...
        ores.clear();
        circ.clear();
        start = Objects.requireNonNull(VertexMain.client.player).getBlockPos();
        toScan.add(start);
        scanned = false;
    }
    ...
    public String getContext() {
        return scannedBlocks.size() + "S|" + new ArrayList<>(this.ores).stream()
                .filter(blockPos -> shouldRenderOre(Objects.requireNonNull(VertexMain.client.world).getBlockState(blockPos).getBlock()))
    ...
    public void onWorldRender(MatrixStack matrices) {
        ...
        Camera cam = VertexMain.client.gameRenderer.getCamera();
        ...
        if (showScanned.getValue()) {
            for (int i = 0; i < real.size(); i++) {
                ...
                double dist = new Vec3d(entry.getKey().getX(),
                        entry.getKey().getY(),
                        entry.getKey().getZ()
                ).distanceTo(Objects.requireNonNull(VertexMain.client.player)
                        .getPos());
        ...
        for (BlockPos ore : new ArrayList<>(this.ores)) {
            ...
            Block t = Objects.requireNonNull(VertexMain.client.world).getBlockState(ore).getBlock();
            if (!shouldRenderOre(t)) {
                continue;
            }
            Vec3d p = new Vec3d(ore.getX(), ore.getY(), ore.getZ());
            double dist = p.distanceTo(Objects.requireNonNull(VertexMain.client.player).getPos());
            dist = MathHelper.clamp(dist, 0, 30);
            Renderer.R3D.renderFilled(p,
                    new Vec3d(1, 1, 1),
                    Renderer.Util.modify(oreColors.containsKey(t) ? oreColors.get(t) : new Color(VertexMain.client.world.getBlockState(ore)
                            .getMapColor(VertexMain.client.world, ore).color), -1, -1, -1, (int) ((dist / 30d) * 200)),
                    matrices
            );
        }
    }
