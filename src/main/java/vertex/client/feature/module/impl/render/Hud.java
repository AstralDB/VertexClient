...
import vertex.client.feature.gui.hud.HudRenderer;
...
import vertex.client.mixin.render.IInGameHudMixin;
import vertex.client.mixin.screen.IDebugHudMixin;
...
public class Hud extends Module {
    ...
    public void onHudRender() {
        if (VertexMain.client.getNetworkHandler() == null) {
            return;
        }
        ...
        if (VertexMain.client.options.debugEnabled) {
            double heightAccordingToMc = 9;
            List<String> lt = ((IDebugHudMixin) ((IInGameHudMixin) VertexMain.client.inGameHud).getDebugHud()).callGetLeftText();
            List<String> rt = ((IDebugHudMixin) ((IInGameHudMixin) VertexMain.client.inGameHud).getDebugHud()).callGetRightText();
    ...
    public void drawTopLeft(MatrixStack ms) {
        ...
        if (this.ping.getValue()) {
            PlayerListEntry ple = Objects.requireNonNull(VertexMain.client.getNetworkHandler())
                    .getPlayerListEntry(Objects.requireNonNull(VertexMain.client.player).getUuid());
            values.add((ple == null || ple.getLatency() == 0 ? "?" : ple.getLatency() + "") + " ms");
        }
        if (this.coords.getValue()) {
            BlockPos bp = Objects.requireNonNull(VertexMain.client.player).getBlockPos();
    ...
    void drawModuleList(MatrixStack ms) {
        double width = VertexMain.client.getWindow().getScaledWidth();
        double y = 0;
