...
public class AutoTrap extends Module {
    ...
    boolean isTrappedAlready(Entity entity) {
        ...
        boolean smallMatches = Arrays.stream(buildOffsetsSmall).allMatch(ints -> {
            ...
            return Objects.requireNonNull(VertexMain.client.world).getBlockState(a).getMaterial().blocksMovement();
        });
        if (smallMatches) {
            return true;
        }
        ...
        for (double[] ints : possibleOffsetsHome) {
            ...
            boolean matches = Arrays.stream(buildOffsetsBig).allMatch(ints1 -> {
                ...
                return VertexMain.client.world.getBlockState(a).getMaterial().blocksMovement();
            });
            if (matches) {
                return true;
            }
    ...
    boolean inHitRange(Entity attacker, Vec3d pos) {
        return attacker.getCameraPosVec(1f).distanceTo(pos) <= Objects.requireNonNull(VertexMain.client.interactionManager).getReachDistance() + .5;
    }

    @Override
    public void onFastTick() {
        for (Entity player : Objects.requireNonNull(VertexMain.client.world).getPlayers()) {
            if (player.equals(VertexMain.client.player)) {
                continue;
            }
            ...
            double[][] filteredPlan = Arrays.stream(planToUse).filter(ints -> {
                ...
                return inHitRange(VertexMain.client.player, v.add(.5, .5, .5));
            }).toList().toArray(double[][]::new);

            int slot = -1;
            for (int i = 0; i < 9; i++) {
                ItemStack real = VertexMain.client.player.getInventory().getStack(i);
                if (real.getItem() instanceof BlockItem bi && Block.isShapeFullCube(bi.getBlock()
                        .getDefaultState()
                        .getOutlineShape(VertexMain.client.world, new BlockPos(0, 0, 0), ShapeContext.absent()))) {
                    slot = i;
                }
            }
            if (slot == -1) {
            ...
            VertexMain.client.execute(() -> {
                int selSlot = VertexMain.client.player.getInventory().selectedSlot;
                VertexMain.client.player.getInventory().selectedSlot = finalSlot;
                for (double[] ints : filteredPlan) {
                    BlockPos current = new BlockPos(pos.add(ints[0], ints[1], ints[2]));
                    if (!VertexMain.client.world.getBlockState(current).isAir()) {
                        continue;
                    }
                ...
                VertexMain.client.player.getInventory().selectedSlot = selSlot;
            });
        }
    }
        ...
        if (isDebuggerEnabled()) {
            for (Entity player : Objects.requireNonNull(VertexMain.client.world).getPlayers()) {
                ...
                for (double[] ints : planToUse) {
                    ...
                    if (!inHitRange(VertexMain.client.player, v3.add(.5, .5, .5))) {
