...
import vertex.client.helper.vertex.DumpVertexConsumer;
import vertex.client.helper.vertex.DumpVertexProvider;
...
public class ESP extends Module {
    ...
    public void onWorldRender(MatrixStack matrices) {
        ...
        for (Entity entity : VertexMain.client.world.getEntities()) {
            if (entity.squaredDistanceTo(VertexMain.client.player) > Math.pow(range.getValue(), 2)) {
                continue;
            }
    ...
    void renderOutline(Entity e, Color color, MatrixStack stack) {
        Vec3d eSource = new Vec3d(
                MathHelper.lerp(VertexMain.client.getTickDelta(), e.prevX, e.getX()),
                MathHelper.lerp(VertexMain.client.getTickDelta(), e.prevY, e.getY()),
                MathHelper.lerp(VertexMain.client.getTickDelta(), e.prevZ, e.getZ())
        );
