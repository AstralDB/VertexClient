...
public class AddonManager {
    public static final File ADDON_DIRECTORY = new File(VertexMain.BASE, "addons");
    ...
    Addon safeLoadAddon(File file) {
        VertexMain.log(Level.INFO, "Attempting to load addon " + file.getName());
        try {

            return loadAddon(file);
        } catch (Throwable e) {
            VertexMain.log(Level.ERROR, "Failed to load " + file.getName());
            e.printStackTrace();
            ...
            if (e instanceof ClassCastException) {
                VertexMain.log(Level.INFO, "This error probably occurs because of an outdated coffee SDK. Please report this error to the addon developer(s).");
            }
        }
        return null;
    }
        ...
        for (File file : Objects.requireNonNull(ADDON_DIRECTORY.listFiles())) {
            ...
            if (file.getName().endsWith(".jar")) {
                if (safeLoadAddon(file) == null) {
                    VertexMain.log(Level.WARN, "Renaming addon to prevent re-discovery because it failed to load");
    ...
    public void enableAddon(Addon addon) {
        ...
        if (customModules != null) {
            for (AddonModule additionalModule : customModules) {
                VertexMain.log(Level.INFO, "Loading module " + additionalModule.getName() + " from addon " + addon.name);
                ModuleRegistry.registerAddonModule(addon, additionalModule);
            }
        }
        if (customCommands != null) {
            for (Command customCommand : customCommands) {
                VertexMain.log(Level.INFO, "Loading command " + customCommand.getName() + " from addon " + addon.name);
    ...
    Addon loadAddon(File location) throws Exception {
        ...
        VertexMain.log(Level.INFO, "Discovered addon " + mainClassA.name + " by " + String.join(", ", mainClassA.developers));
