...
import vertex.client.helper.util.NbtFormatter;
...
public class NbtEditorScreen extends ClientScreen implements FastTickable {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        if (InputUtil.isKeyPressed(VertexMain.client.getWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            scrollX -= amount * 10;
        } else {
            scroll -= amount * 10;
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (isPaste(keyCode)) {
            String clip = VertexMain.client.keyboard.getClipboard();
            skipAppend = true;
