...
import vertex.client.feature.module.impl.render.Hud;
...
public class SpeedHud extends HudElement {
    ...
    public SpeedHud() {
        super("Speed", VertexMain.client.getWindow().getScaledWidth() / 2d - 160 / 2d, VertexMain.client.getWindow().getScaledHeight() - 40 - 64, 160, 64);
    }
    ...
    public void fastTick() {
        if (update.hasExpired(50)) { // update when velocity gets updated
            double speedCombined = VertexMain.client.player.getVelocity().length();
