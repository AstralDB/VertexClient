...
public abstract class HudElement {
    ...
    public void mouseDragged(double deltaX, double deltaY) {
        if (selected) {
            this.posX += deltaX;
            this.posY += deltaY;
            this.posX = MathHelper.clamp(this.posX, 0, VertexMain.client.getWindow().getScaledWidth() - this.width);
            this.posY = MathHelper.clamp(this.posY, 0, VertexMain.client.getWindow().getScaledHeight() - this.height);
        }
    }
